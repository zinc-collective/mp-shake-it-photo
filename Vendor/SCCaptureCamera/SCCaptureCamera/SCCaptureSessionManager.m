...
- (void)addStillImageOutput {
    ...
    if (NSFoundationVersionNumber > NSFoundationVersionNumber_iOS_7_1) {
        tmpOutput.highResolutionStillImageOutputEnabled = YES;
    }
    
    

    NSLog(@"version: %f %i",NSFoundationVersionNumber,NSFoundationVersionNumber == NSFoundationVersionNumber_iOS_7_0);
...
//    [videoConnection setVideoOrientation:avcaptureOrientation];
    ...
    [_stillImageOutput captureStillImageAsynchronouslyFromConnection:videoConnection completionHandler:^(CMSampleBufferRef imageDataSampleBuffer, NSError *error) {
        ...
        SCDLog(@"originImage:%@ %f", [NSValue valueWithCGSize:image.size],image.scale);
//        [SCCommon saveImageToPhotoAlbum:image];
        
        CGFloat squareLength = image.size.width;// SC_APP_SIZE.width;
        CGFloat headHeight = 0;//_previewLayer.bounds.size.height - squareLength;//_previewLayer的frame是(0, 44, 320, 320 + 44)
        CGSize size = CGSizeMake(squareLength, squareLength);
        size.height *= (_previewLayer.bounds.size.height / _previewLayer.bounds.size.width);
        
        UIImage *scaledImage = [image resizedImageWithContentMode:UIViewContentModeScaleAspectFill bounds:size interpolationQuality:kCGInterpolationNone];
        ...
        //UIImage *croppedImage = [UIImage imageNamed:@"touch_focus"];

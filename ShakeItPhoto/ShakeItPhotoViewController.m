...
@synthesize fasterShakingCell = _fasterShakingCell;
@synthesize polaroidBorderCell = _polaroidBorderCell;
...
- (UITableViewCell*) tableView: (UITableView*) tableView cellForRowAtIndexPath: (NSIndexPath*) indexPath
{
    ...
    switch(indexPath.section)
    {
        case 0:
        {
            if(indexPath.row == 0)
			{
                cell = self.polaroidBorderCell;
			}
            else if(indexPath.row == 1)
            {
                cell = self.fasterShakingCell;
            }
            else if(indexPath.row == 2)
            {
                cell = self.keepOriginalCell;
            }
...
- (void) setupOptions
{
	...
	aSwitch = (UISwitch*)[self.fasterShakingCell.contentView viewWithTag: 1];
	[aSwitch setOn: [defaults boolForKey: kShakeItPhotoFasterShakingKey] animated: YES];

	aSwitch = (UISwitch*)[self.polaroidBorderCell.contentView viewWithTag: 1];
	[aSwitch setOn: [defaults boolForKey: kShakeItPhotoPolaroidBorderKey] animated: YES];
}
